...
public class VariantSoldCount {
    ...
    public static VariantSoldCount create(
            Draft draft) {
        if (draft == null) {
        ...
        return VariantSoldCount.builder().id(draft.id()).soldCount(draft.soldCount()).version(new Version(null))
                .build();
    }

    public static VariantSoldCount reconstitue(
            Snapshot snapshot) {
        if (snapshot == null) {
        ...
        return VariantSoldCount.builder().id(snapshot.id()).soldCount(snapshot.soldCount()).version(snapshot.version())
                .build();
    }

    public VariantSoldCount applyUpdateInfo(
            UpdateInfo u) {
        if (u == null)
