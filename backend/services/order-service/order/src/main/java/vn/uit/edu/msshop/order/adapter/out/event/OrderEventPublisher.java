...
public class OrderEventPublisher implements PublishOrderEventPort {
     ...
     * OrderCreated(outboxEvent.getEventId(),outboxEvent.getCurrency(),
     * outboxEvent.getOrderId(), outboxEvent.getPaymentMethod(),
     * outboxEvent.getPaymentValue(), outboxEvent.getUserId(),
     * outboxEvent.getUserEmail());
     * Message<OrderCreated>
     * message=MessageBuilder.withPayload(event).setHeader(KafkaHeaders.TOPIC,
     * ORDER_CREATED_TOPIC).build();
     * try {
    ...
    @Override
    public void publishClearCartEvent(
            OrderCreatedSuccessDocument outboxEvent) {
        OrderCreatedSuccess event = new OrderCreatedSuccess(outboxEvent.getEventId(), outboxEvent.getUserId(),
                outboxEvent.getVariantIds());
        Message<OrderCreatedSuccess> message = MessageBuilder.withPayload(event)
                .setHeader(KafkaHeaders.TOPIC, CLEAR_CART_TOPIC).build();
        try {
    ...
    @Override
    public void publishOrderUpdatedEvent(
        ...
        Message<OrderUpdatedEvent> message = MessageBuilder.withPayload(orderUpdatedEvent)
