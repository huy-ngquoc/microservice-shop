...
public class OrderProductEventListener {
    ...
    public void onVariantUpdate(
        ...
        if (v == null)
            v = new VariantInfo(event.getVariantId(), event.getProductId(), event.getProductName(), event.getPrice(),
                    event.getTraits(), event.getImageKey());
        else {
    ...
    public void onProductCreated(
        ...
        if (eventDocumentRepo.existsById(event.getEventId()))
            return;
        List<VariantInfo> infos = event.getVariantCreateds().stream().map(item -> new VariantInfo(item.getVariantId(),
                event.getProductId(), event.getProductName(), item.getPrice(), item.getTraits(), item.getImageKey()))
                .toList();
