...
public class UpdateVariantSoldCountService implements UpdateVariantSoldCountUseCase {
    ...
    private VariantSoldCount updateSoldCount(
            UpdateVariantSoldCountCommand command) {
        ...
        final var updateInfo = VariantSoldCount.UpdateInfo.builder().id(command.id()).soldCount(command.soldCount())
                .build();
    ...
    public List<VariantSoldCountView> updateMany(
        ...
        List<VariantSoldCount> listSoldCounts = findPort
                .findByIdInList(commands.stream().map(IncreaseVariantSoldCountCommand::id).toList());
        ...
        for (IncreaseVariantSoldCountCommand command : commands) {
            ...
            if (v == null) {
                final var updateCommand = new UpdateVariantSoldCountCommand(command.id(),
                        new SoldCount(command.amount().value()));
                toSaves.add(updateSoldCount(updateCommand));
            } else {
                final var updateCommand = new UpdateVariantSoldCountCommand(command.id(),
                        new SoldCount(command.amount().value() + v.getSoldCount().value()));
                toSaves.add(updateSoldCount(updateCommand));
            }
        }
    ...
    private VariantSoldCount findByIdInList(
            VariantId id,
            List<VariantSoldCount> soldCounts) {
