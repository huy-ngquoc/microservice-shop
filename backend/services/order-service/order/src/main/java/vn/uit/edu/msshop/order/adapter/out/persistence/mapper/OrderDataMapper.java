...
public class OrderDataMapper {
    ...
    public ShippingInfo toShippingInfoDomain(
            ShippingInfoDocument document) {
        return new ShippingInfo(document.getFullName(), document.getAddress(), document.getPhone(),
                document.getEmail());
    }
    ...
    public OrderDetail toOrderDetailDomain(
        ...
        return new OrderDetail(orderDetailDocument.getVariantId(), orderDetailDocument.getProductId(),
                orderDetailDocument.getProductName(), orderDetailDocument.getImageKey(),
                orderDetailDocument.getAmount(), orderDetailDocument.getUnitPrice(), orderDetailDocument.getTraits());
    }
    ...
    public OrderDocument toDocument(
        ...
        return new OrderDocument(order.getId().value(), shippingInfoDocument, details, order.getStatus().value(),
                ...
                order.getDiscount().value(), order.getTotalPrice().value(), order.getCreateAt().value(),
                order.getUpdateAt().value(), order.getVersion() == null ? null : order.getVersion().value(),
                order.getCurrency().value(), order.getPaymentMethod().value(), order.getPaymentStatus().value());
    }

    public ShippingInfoDocument toShippingInfoDocument(
            ShippingInfo shippingInfo) {
        return new ShippingInfoDocument(shippingInfo.fullName(), shippingInfo.address(), shippingInfo.email(),
                shippingInfo.phone());
    }

    public OrderDetailDocument toOrderDetailDocument(
            OrderDetail orderDetail) {
        return new OrderDetailDocument(orderDetail.variantId(), orderDetail.productName(), orderDetail.traits(),
                orderDetail.imageKey(), orderDetail.productId(), orderDetail.amount(), orderDetail.unitPrice());
    }
}
