...
public class OrderEventListener {
    ...
    public void onPaymentExpired(
        ...
        if (!eventDocumentRepo.existsById(event.eventId())) {
            Order order = loadPort.loadById(new OrderId(event.orderId()))
                    .orElseThrow(() -> new OrderNotFoundException(new OrderId(event.orderId())));
            String oldStatus = order.getStatus().value();
            Order.UpdateInfo updateInfo = Order.UpdateInfo.builder().id(order.getId())
                    .shippingInfo(order.getShippingInfo()).orderStatus(new OrderStatus("PAYMENT_EXPIRED")).build();
    ...
    public void onOnlinePaymentSuccess(
        ...
        if (!eventDocumentRepo.existsById(event.getEventId())) {
            Order order = loadPort.loadById(new OrderId(event.getOrderId()))
                    .orElseThrow(() -> new OrderNotFoundException(new OrderId(event.getOrderId())));
            Order.UpdateInfo updateInfo = Order.UpdateInfo.builder().id(order.getId())
                    .shippingInfo(order.getShippingInfo()).orderStatus(order.getStatus()).build();
            final var saved = order.applyUpdateInfo(updateInfo).updatePaymentStatus(new PaymentStatus("SUCCESS"))
                    .updateStatus(new OrderStatus("CONFIRMED"));
