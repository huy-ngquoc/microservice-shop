...
public class VariantSoldCountDataMapper {
    public VariantSoldCount toDomain(
            VariantSoldCountDocument document) {
        final var snapshot = VariantSoldCount.Snapshot.builder().id(new VariantId(document.getId()))
                .soldCount(new SoldCount(document.getSoldCount())).version(new Version(document.getVersion())).build();
        return VariantSoldCount.reconstitue(snapshot);
    }

    public VariantSoldCountDocument toDocument(
            VariantSoldCount domain) {
        return new VariantSoldCountDocument(domain.getId().value(), domain.getSoldCount().value(),
                domain.getVersion() == null ? null : domain.getVersion().value());
    }
}
