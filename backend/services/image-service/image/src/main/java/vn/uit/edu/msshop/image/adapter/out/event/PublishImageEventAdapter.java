...
public class PublishImageEventAdapter implements PublishImageEventPort {
    ...
    @Override
    public void publishImageRemoveSuccess(
            ImageRemoveSuccessDocument outboxEvent) {
        final var event = new ImageRemoveSuccess(outboxEvent.getEventId(), outboxEvent.getUrl(),
                outboxEvent.getPublicId(),
                outboxEvent.getFileName(), outboxEvent.getWidth(), outboxEvent.getHeight(), outboxEvent.getSize(),
                outboxEvent.getObjectId());
