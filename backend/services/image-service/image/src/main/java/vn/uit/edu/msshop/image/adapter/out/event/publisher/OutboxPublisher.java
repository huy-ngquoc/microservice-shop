...
public class OutboxPublisher {
    ...
    public void publishPendingEvents() {
        List<ImageRemoveSuccessDocument> pendingEvents = imageRemoveSuccessDocumentRepo
                .findTop50ByStatusOrderByCreatedAtAsc("PENDING");

        for (ImageRemoveSuccessDocument event : pendingEvents) {
            try {
                ImageRemoveSuccess imageRemoveSuccess = new ImageRemoveSuccess(event.getEventId(), event.getUrl(),
                        event.getPublicId(), event.getFileName(), event.getWidth(), event.getHeight(), event.getSize(),
                        event.getObjectId());
    ...
    private void updateStatus(
            ImageRemoveSuccessDocument event,
            String status,
            String error) {
