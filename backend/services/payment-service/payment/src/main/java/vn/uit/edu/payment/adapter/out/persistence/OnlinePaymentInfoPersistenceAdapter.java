...
public class OnlinePaymentInfoPersistenceAdapter implements LoadOnlinePaymentInfoPort, SaveOnlinePaymentInfoPort {
    ...
    @Override
    public String getPaymentLinkByOrderId(OrderId orderId) {
        Payment p = loadPaymentPort.loadPaymentByOrderId(orderId);
        if(p==null) throw new PaymentNotFoundException(orderId);
        final var entity = repo.findById(p.getPaymentId().value()).orElseThrow(()->new OnlinePaymentInfoNotFoundException(p.getPaymentId()));
