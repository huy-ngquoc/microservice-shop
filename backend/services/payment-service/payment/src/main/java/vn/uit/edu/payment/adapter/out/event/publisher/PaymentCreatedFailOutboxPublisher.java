...
public class PaymentCreatedFailOutboxPublisher {
    ...
    public void publishPendingEvents() {
        List<PaymentCreatedFailDocument> pendingEvents = paymentCreatedFailRepo.findTop50ByEventStatusOrderByCreatedAtAsc("PENDING");

        for (PaymentCreatedFailDocument event : pendingEvents) {
            try {
                PaymentCreatedFail paymentCreatedFail= new PaymentCreatedFail(event.getEventId(), event.getOrderId(), event.getUserId(), event.getUserEmail());
    ...
    private void updateStatus(PaymentCreatedFailDocument event, String status, String error) {
    ...
    @Transactional
    public void markAsSent(PaymentCreatedFailDocument event) {
        updateStatus(event, "SENT", null);
    }
    private void handleFailure(PaymentCreatedFailDocument event, String error) {
