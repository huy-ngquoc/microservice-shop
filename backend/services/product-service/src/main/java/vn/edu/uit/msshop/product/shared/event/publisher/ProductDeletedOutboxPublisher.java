...
public class ProductDeletedOutboxPublisher {
  ...
  public void publishPendingEvents() {
    List<ProductDeletedDocument> pendingEvents =
        productDeletedRepo.findTop50ByEventStatusOrderByCreatedAtAsc("PENDING");

    for (ProductDeletedDocument event : pendingEvents) {
      try {

        ProductDeleted productDeleted =
            new ProductDeleted(event.getEventId(), event.getProductId());
        kafkaTemplate.send(PUBLISH_TOPIC, productDeleted).whenComplete((result, ex) -> {
          if (ex == null) {
