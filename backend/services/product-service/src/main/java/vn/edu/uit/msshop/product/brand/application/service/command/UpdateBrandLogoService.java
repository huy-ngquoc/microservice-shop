...
public class UpdateBrandLogoService implements UpdateBrandLogoUseCase {
  ...
  @Override
  @Transactional
  public BrandLogoView updateLogo(UpdateBrandLogoCommand command) {
    ...
    final var event = new BrandLogoUpdated(saved.getId(), saved.getLogoKey(), brand.getLogoKey());
  ...
  private @Nullable Brand commitImageChange(final Brand current, final BrandLogoKey newLogoKey) {
    ...
    try {
      final var next = new Brand(current.getId(), current.getName(), newLogoKey,
          current.getVersion(), current.getDeletionTime());
  ...
  private void ensureLogoKeyExistsInTemp(final BrandLogoKey logoKey) {
    if (!this.logoStoragePort.existsAsTemp(logoKey)) {
      throw new BrandLogoKeyNotFoundException(logoKey);
    }
  }

  private void deleteOldLogo(@Nullable final BrandLogoKey oldKey) {
    if (oldKey == null) {
      return;
    }
