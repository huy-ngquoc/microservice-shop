...
public class VariantDeletedOutboxPublisher {
  ...
  public void publishPendingEvents() {
    List<VariantDeletedDocument> pendingEvents =
        variantDeletedRepo.findTop50ByEventStatusOrderByCreatedAtAsc("PENDING");

    for (VariantDeletedDocument event : pendingEvents) {
      try {
        VariantDeleted variantDeleted =
            new VariantDeleted(event.getEventId(), event.getVariantId());
        kafkaTemplate.send(PUBLISH_TOPIC, variantDeleted).whenComplete((result, ex) -> {
          if (ex == null) {
  ...
  private void updateStatus(VariantDeletedDocument event, String status, @Nullable String error) {
  ...
  private void handleFailure(VariantDeletedDocument event, @Nullable String error) {
