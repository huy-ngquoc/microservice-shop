...
public class ProductViewMapper {
  public ProductView toView(final Product product, final ProductSoldCount soldCount,
      final ProductStockCount stockCount, final ProductRating rating) {
    final var priceRange = product.getPriceRange();

    return new ProductView(product.getId().value(), product.getName().value(),
        product.getCategoryId().value(), product.getBrandId().value(),
        priceRange.minPrice().value(), priceRange.maxPrice().value(), soldCount.getValue().value(),
        stockCount.getValue().value(), rating.getAverage().value(), rating.getAmount().value(),
        product.getOptions().unwrap(), this.toView(product.getVariants()),
        product.getImageKeys().unwrap(), product.getVersion().value());
  }

  public List<ProductVariantView> toView(final ProductVariants variants) {
    return variants.values().stream().map(this::toView).toList();
  }

  public ProductVariantView toView(final ProductVariant variant) {
    return new ProductVariantView(variant.id().value(), variant.price().value(),
