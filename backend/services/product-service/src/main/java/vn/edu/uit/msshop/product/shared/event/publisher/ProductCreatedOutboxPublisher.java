...
public class ProductCreatedOutboxPublisher {
  ...
  public void publishPendingEvents() {
    List<ProductCreatedDocument> pendingEvents =
        productCreatedRepo.findTop50ByEventStatusOrderByCreatedAtAsc("PENDING");

    for (ProductCreatedDocument event : pendingEvents) {
      try {
        List<VariantCreated> variantCreateds = event.getVariantCreateds().stream()
            .map(item -> new VariantCreated(item.getVariantId(), event.getProductId(),
                event.getProductName(), item.getPrice(), item.getTraits(), item.getImageKey()))
            .toList();
        ProductCreated productCreated = new ProductCreated(event.getEventId(), event.getProductId(),
            event.getProductName(), variantCreateds);
        kafkaTemplate.send(PUBLISH_TOPIC, productCreated).whenComplete((result, ex) -> {
          if (ex == null) {
  ...
  private void updateStatus(ProductCreatedDocument event, String status, @Nullable String error) {
  ...
  private void handleFailure(ProductCreatedDocument event, @Nullable String error) {
