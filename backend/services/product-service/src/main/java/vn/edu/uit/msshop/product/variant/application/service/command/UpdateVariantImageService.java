...
public class UpdateVariantImageService implements UpdateVariantImageUseCase {
  ...
  @Override
  @Transactional
  public VariantImageView updateImage(final UpdateVariantImageCommand command) {
    ...
    final var event =
        new VariantImageUpdated(saved.getId(), saved.getImageKey(), variant.getImageKey());
  ...
  private @Nullable Variant commitImageChange(final Variant current,
    ...
    try {
      final var next = new Variant(current.getId(), current.getProductId(),
          current.getProductName(), current.getPrice(), current.getTraits(), current.getTargets(),
          newImageKey, current.getVersion(), current.getDeletionTime());
  ...
  private void ensureImageKeyExistsInTemp(final VariantImageKey imageKey) {
    if (!this.imageStoragePort.existsAsTemp(imageKey)) {
      throw new VariantImageKeyNotFoundException(imageKey);
    }
  }

  private void deleteOldImage(@Nullable final VariantImageKey oldKey) {
    if (oldKey == null) {
      return;
    }
