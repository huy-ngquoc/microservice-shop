...
public class UpdateProductInfoService implements UpdateProductInfoUseCase {
  ...
  @Override
  @Transactional
  public ProductView updateInfo(final UpdateProductInfoCommand command) {
    ...
    final var next = this.applyChanges(product, nameSet, categoryIdSet, brandIdSet);
    if (next == null) {
      return this.mapper.toView(product, soldCount, stockCount, rating);
    }
  ...
  private @Nullable Product applyChanges(final Product current,
    ...
    return new Product(current.getId(), newName, newCategoryId, newBrandId,
        current.getConfiguration(), current.getImageKeys(), current.getVersion(),
  ...
  private void syncProductNameToVariantsIfChanged(final Product before, final Product after) {
    if (after.getName().equals(before.getName())) {
      return;
    }

    this.updateVariantProductNamePort.updateProductNameByProductId(after.getId(), after.getName());
  }
}
