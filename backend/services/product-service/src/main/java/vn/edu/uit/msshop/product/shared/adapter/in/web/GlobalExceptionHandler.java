...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiErrorResponse> handleNotFound(final NotFoundException ex,
    ...
    final var message = String.format("Resource not found: %s", ex.getMessage());

    log.debug(message);

    final var status = HttpStatus.NOT_FOUND;
    final var response = GlobalExceptionHandler.buildResponse(status, message, request);

    return ResponseEntity.status(status).body(response);
  }

  @ExceptionHandler(BusinessRuleException.class)
  public ResponseEntity<ApiErrorResponse> handleBusinessRule(final BusinessRuleException ex,
      final HttpServletRequest request) {
    final var message = String.format("Business rule violation: %s", ex.getMessage());
  ...
  public ResponseEntity<ApiErrorResponse> handleOptimisticLock(final OptimisticLockException ex,
      final HttpServletRequest request) {
    final var message = String.format("Optimistic lock conflict: %s", ex.getMessage());
  ...
  public ResponseEntity<Object> handleTypeMismatch(final MethodArgumentTypeMismatchException ex,
    ...
    final var message =
        String.format("The parameter '%s' should be of type %s", ex.getName(), typeName);
  ...
  public ResponseEntity<ApiErrorResponse> handleValidation(final MethodArgumentNotValidException ex,
    ...
    final var fieldErrors =
        ex.getBindingResult().getFieldErrors().stream().map(fe -> new ValidationError(fe.getField(),
            Objects.requireNonNullElse(fe.getDefaultMessage(), "Validation error"))).toList();

    final var message = String.format("Validation failed: %s", fieldErrors);

    log.debug(message);

    final var status = HttpStatus.BAD_REQUEST;
    final var response = new ApiErrorResponse(status.value(), status.getReasonPhrase(), message,
  ...
  public ResponseEntity<ApiErrorResponse> handleConstraintViolation(
    ...
    final var errors = ex.getConstraintViolations().stream()
        .map(cv -> new ValidationError(cv.getPropertyPath().toString(), cv.getMessage())).toList();

    final var message = String.format("Constraint violation: %s", errors);

    log.debug(message);

    final var status = HttpStatus.BAD_REQUEST;
    final var response = new ApiErrorResponse(status.value(), status.getReasonPhrase(), message,
        Instant.now(), request.getRequestURI(), errors);
  ...
  public ResponseEntity<ApiErrorResponse> handleNotReadable(
    ...
    final var message = String.format("Malformed request body: %s", ex.getMessage());
  ...
  public ResponseEntity<ApiErrorResponse> handleMissingParam(
    ...
    final var message = String.format("Missing request parameter: %s", ex.getMessage());
  ...
  public ResponseEntity<ApiErrorResponse> handleDomainException(final DomainException ex,
      final HttpServletRequest request) {
    log.warn("Domain invariant violation", ex);

    final var status = HttpStatus.INTERNAL_SERVER_ERROR;
    final var response =
        GlobalExceptionHandler.buildResponse(status, "Internal server error", request);
  ...
  private static ApiErrorResponse buildResponse(final HttpStatus status, final String message,
      final HttpServletRequest request) {
