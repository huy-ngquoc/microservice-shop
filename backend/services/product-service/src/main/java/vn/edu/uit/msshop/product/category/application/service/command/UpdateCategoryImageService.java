...
public class UpdateCategoryImageService implements UpdateCategoryImageUseCase {
  ...
  @Override
  @Transactional
  public CategoryImageView updateImage(final UpdateCategoryImageCommand command) {
    ...
    final var event =
        new CategoryImageUpdated(saved.getId(), saved.getImageKey(), category.getImageKey());
  ...
  private @Nullable Category commitImageChange(final Category current,
    ...
    try {
      final var next = new Category(current.getId(), current.getName(), newImageKey,
  ...
  private void ensureImageKeyExistsInTemp(final CategoryImageKey imageKey) {
    if (!this.imageStoragePort.existsAsTemp(imageKey)) {
      throw new CategoryImageKeyNotFoundException(imageKey);
    }
  }

  private void deleteOldImage(@Nullable final CategoryImageKey oldKey) {
    if (oldKey == null) {
      return;
    }
