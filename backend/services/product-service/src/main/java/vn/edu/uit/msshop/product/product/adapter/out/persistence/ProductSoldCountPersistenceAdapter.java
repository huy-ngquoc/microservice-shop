...
@Slf4j
public class ProductSoldCountPersistenceAdapter implements LoadProductSoldCountPort,
    LoadAllProductSoldCountsPort, InitializeProductSoldCountPort, IncreaseAllProductSoldCountsPort,
    DecreaseAllProductSoldCountsPort, DeleteProductSoldCountPort {
  private static final Collector<ProductSoldCount, ?, Map<ProductId, ProductSoldCount>> COLLECTOR =
      Collectors.toUnmodifiableMap(ProductSoldCount::getId, Function.identity(),
  ...
  @Override
  public ProductSoldCount loadByIdOrZero(final ProductId id) {
  ...
  @Override
  public Map<ProductId, ProductSoldCount> loadAllByIds(final Set<ProductId> ids) {
    if (ids.isEmpty()) {
      return Map.of();
    }
  ...
  @Override
  public ProductSoldCount initialize(final ProductId id) {
    final var jpaId = id.value();

    final var query = new Query(Criteria.where("_id").is(jpaId));
    final var update = new Update().setOnInsert(ProductSoldCountDocument.Fields.soldCount, 0)
  ...
  @Override
  public void increaseAll(Map<ProductId, Integer> incrementByProductId) {
    ...
    for (final var entry : incrementByProductId.entrySet()) {
      ...
      final var update =
          new Update().inc(ProductSoldCountDocument.Fields.soldCount, entry.getValue())
  ...
  public void decreaseAll(Map<ProductId, Integer> decrementByProductId) {
    ...
    for (final var entry : decrementByProductId.entrySet()) {
      ...
      final var query = new Query(
          Criteria.where("_id").is(jpaId).and(ProductSoldCountDocument.Fields.soldCount).gte(dec));
      final var update = new Update().inc(ProductSoldCountDocument.Fields.soldCount, -dec)
          .set(ProductSoldCountDocument.Fields.lastUpdatedTime, instantNow);
  ...
  private ProductSoldCount upsertAndReturnDomain(final Query query, final Update update) {
    final var options = FindAndModifyOptions.options().returnNew(true).upsert(true);
    final var doc =
        this.mongoTemplate.findAndModify(query, update, options, ProductSoldCountDocument.class);
