...
public class InventoryStockCountFeignAdapter implements FetchInventoryStockCountsPort {
  ...
  @Override
  public List<VariantInventoryStockCount> fetchAll(final Instant rangeStartTime,
    ...
    do {
      final var pageResponse = this.feignClient.getStockCounts(request, pageNumber, PAGE_SIZE);

      List<VariantInventoryStockCount> mappedItems =
          pageResponse.items().stream().map(InventoryStockCountFeignAdapter::toDomainDto).toList();
  ...
  private static VariantInventoryStockCount toDomainDto(final StockCountResponse response) {
    return new VariantInventoryStockCount(new VariantId(response.variantId()),
