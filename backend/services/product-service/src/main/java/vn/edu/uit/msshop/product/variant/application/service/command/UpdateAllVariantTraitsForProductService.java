...
public class UpdateAllVariantTraitsForProductService
  ...
  @Override
  @Transactional
  public void updateTraitsByIds(Map<VariantId, VariantTraits> newTraitsMap) {
    ...
    final var next = variantById.values().stream()
        .map(v -> withNewTraits(v, newTraitsMap.get(v.getId()))).toList();
  ...
  private static Variant withNewTraits(final Variant variant,
    ...
    return new Variant(variant.getId(), variant.getProductId(), variant.getProductName(),
        variant.getPrice(), newTraits, variant.getTargets(), variant.getImageKey(),
        variant.getVersion(), variant.getDeletionTime());
  }
}
