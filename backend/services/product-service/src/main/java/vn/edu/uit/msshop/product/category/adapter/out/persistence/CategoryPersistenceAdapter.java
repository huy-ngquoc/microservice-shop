...
public class CategoryPersistenceAdapter implements ListCategoriesPort,
    ListSoftDeletedCategoriesPort, LoadCategoryPort, LoadSoftDeletedCategoryPort,
    CheckCategoryExistsPort, CreateCategoryPort, UpdateCategoryPort, DeleteCategoryPort {
  private final CategoryMongoRepository repository;
  private final CategoryPersistenceMapper mapper;

  @Override
  public PageResponseDto<Category> list(final PageRequestDto pageRequest) {
    final var pageable = PageRequests.toPageable(pageRequest, CategoryDocument.Fields.id);
    final var page = this.repository.findAllByDeletionTimeIsNull(pageable);

    final var categories = page.getContent().stream().map(this.mapper::toDomain).toList();

    return new PageResponseDto<>(categories, page.getNumber(), page.getSize(),
        page.getTotalElements());
  }

  @Override
  public PageResponseDto<Category> listSoftDeleted(final PageRequestDto pageRequest) {
    final var pageable = PageRequests.toPageable(pageRequest, CategoryDocument.Fields.id);
  ...
  @Override
  public Optional<Category> loadById(final CategoryId id) {
    final var jpaId = id.value();
    return this.repository.findByIdAndDeletionTimeIsNull(jpaId).map(this.mapper::toDomain);
  }

  @Override
  public Optional<Category> loadSoftDeletedById(final CategoryId id) {
    final var jpaId = id.value();
  ...
  @Override
  public Category create(final NewCategory newCategory) {
