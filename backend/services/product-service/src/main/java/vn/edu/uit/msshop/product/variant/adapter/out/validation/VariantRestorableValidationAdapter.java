...
public class VariantRestorableValidationAdapter
    ...
    public void validateRestorable(
        ...
        final var variantTraitsNormalized = variant.getTraits().values().stream()
                .map(t -> t.value().toLowerCase(Locale.ROOT))
                .toList();

        final var duplicateExists = product.variants().stream()
                .anyMatch(v -> {
                    final var existingNormalized = v.traits().stream()
