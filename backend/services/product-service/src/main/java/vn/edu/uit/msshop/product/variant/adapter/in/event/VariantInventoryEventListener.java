...
@Component
@KafkaListener(topics = "inventory-variant", groupId = "product-service")
@RequiredArgsConstructor
public class VariantInventoryEventListener {
  private final SetAllVariantStockCountsUseCase setAllUseCase;

  @KafkaHandler
  public void onSetStockCounts(final SetVariantStockCountsEvent event) {
    final var stockCounts =
        event.details().stream().map(VariantInventoryEventListener::toInventoryStockCount).toList();
  ...
  private static VariantInventoryStockCount toInventoryStockCount(
    ...
    return new VariantInventoryStockCount(new VariantId(detail.variantId()),
