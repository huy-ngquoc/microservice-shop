...
public class ProductPersistenceAdapter
    implements ListProductsPort, ListSoftDeletedProductsPort, LoadProductPort, LoadAllProductsPort,
    LoadSoftDeletedProductPort, CheckProductExistsPort, CheckProductExistsByBrandPort,
    CheckProductExistsByCategoryPort, CheckSoftDeletedProductExistsByBrandPort,
    CheckSoftDeletedProductExistsByCategoryPort, CheckProductExistsByVariantPort, CreateProductPort,
    UpdateProductPort, UpdateAllProductsPort, DeleteProductPort {
  private final ProductMongoRepository repository;
  private final ProductPersistenceMapper mapper;

  @Override
  public PageResponseDto<Product> list(final PageRequestDto pageRequest) {
    ...
    return new PageResponseDto<>(products, page.getNumber(), page.getSize(),
        page.getTotalElements());
  }

  @Override
  public PageResponseDto<Product> listSoftDeleted(final PageRequestDto pageRequest) {
    final var pageable = PageRequests.toPageable(pageRequest, ProductDocument.Fields.id);
  ...
  @Override
  public Optional<Product> loadById(final ProductId id) {
    final var jpaId = id.value();
    return this.repository.findByIdAndDeletionTimeIsNull(jpaId).map(this.mapper::toDomain);
  }

  @Override
  public List<Product> loadAllByIds(final Collection<ProductId> ids) {
    final var jpaIds = ids.stream().map(ProductId::value).toList();
    return this.repository.findAllByDeletionTimeIsNull(jpaIds).stream().map(this.mapper::toDomain)
        .toList();
  }

  @Override
  public Optional<Product> loadSoftDeletedById(final ProductId id) {
    final var jpaId = id.value();
  ...
  @Override
  public boolean existsByCategoryId(final ProductCategoryId categoryId) {
  ...
  @Override
  public boolean existsSoftDeletedByCategoryId(final ProductCategoryId categoryId) {
    final var jpaCategoryId = categoryId.value();
    return this.repository.existsByCategoryIdAndDeletionTimeIsNotNull(jpaCategoryId);
  }

  @Override
  public boolean existsByVariantId(final ProductVariantId variantId) {
    final var jpaVariantId = variantId.value();
    return this.repository.existsByVariants_Id(jpaVariantId);
  }

  @Override
  public Product create(final NewProduct newProduct) {
  ...
  @Override
  public List<Product> updateAll(final Collection<Product> products) {
    if (products.isEmpty()) {
      return List.of();
    }

    final var toSaveAll = products.stream().map(this.mapper::toPersistence).toList();
    ...
    return savedAll.stream().map(this.mapper::toDomain).toList();
  }
