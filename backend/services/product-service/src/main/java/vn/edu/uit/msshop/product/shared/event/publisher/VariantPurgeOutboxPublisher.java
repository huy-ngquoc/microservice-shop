...
public class VariantPurgeOutboxPublisher {
  ...
  public void publishPendingEvents() {
    List<VariantPurgeDocument> pendingEvents =
        variantPurgeRepo.findTop50ByEventStatusOrderByCreatedAtAsc("PENDING");
    ...
    for (VariantPurgeDocument event : pendingEvents) {
      try {
        ...
        kafkaTemplate.send(PUBLISH_TOPIC, variantPurge).whenComplete((result, ex) -> {
          if (ex == null) {
