...
public interface CreateProductUseCase {
  ProductView create(final CreateProductCommand command);

  default ProductView createSimple(final CreateSimpleProductCommand command) {
    final var defaultNewProductVariant =
        new NewProductVariant(command.price(), ProductVariantTraits.empty(), command.targets());

    final var defaultNewProductVariants = new NewProductVariants(List.of(defaultNewProductVariant));

    final var defaultNewProductConfiguration =
        new NewProductConfiguration(ProductOptions.empty(), defaultNewProductVariants);
