...
public class DeleteVariantImageService implements DeleteVariantImageUseCase {
  ...
  @Override
  @Transactional
  public VariantImageView deleteImage(final DeleteVariantImageCommand command) {
    ...
    final var next = new Variant(variant.getId(), variant.getProductId(), variant.getProductName(),
        variant.getPrice(), variant.getTraits(), variant.getTargets(), null, expectedVersion,
        variant.getDeletionTime());
    final var saved = this.updatePort.update(next);

    final var event = new VariantImageUpdated(saved.getId(), null, oldKey);
  ...
  private void deleteOldImage(final VariantImageKey oldKey) {
    try {
