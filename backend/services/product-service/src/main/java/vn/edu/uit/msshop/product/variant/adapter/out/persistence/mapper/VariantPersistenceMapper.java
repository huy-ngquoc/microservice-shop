...
public class VariantPersistenceMapper {
  public Variant toDomain(final VariantDocument entity) {
    ...
    final var versionValue =
        Objects.requireNonNull(entity.getVersion(), "Persisted brand must have a version");
    ...
    return new Variant(id, productId, productName, price, traits, targets, imageKey, version,
        deletionTime);
  }

  public VariantDocument toPersistence(final Variant variant) {
    return new VariantDocument(variant.getId().value(), variant.getProductId().value(),
        variant.getProductName().value(), variant.getPrice().value(), variant.getTraits().unwrap(),
        variant.getTargets().unwrap(), VariantImageKey.unwrap(variant.getImageKey()),
        variant.getVersion().value(), VariantDeletionTime.unwrap(variant.getDeletionTime()));
  }

  public VariantDocument toPersistence(final NewVariant newVariant) {
    return new VariantDocument(newVariant.getId().value(), newVariant.getProductId().value(),
        newVariant.getProductName().value(), newVariant.getPrice().value(),
        newVariant.getTraits().unwrap(), newVariant.getTargets().unwrap(), null, null, null);
  }
}
