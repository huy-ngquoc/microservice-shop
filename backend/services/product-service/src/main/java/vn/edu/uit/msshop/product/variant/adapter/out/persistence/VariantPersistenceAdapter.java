...
public class VariantPersistenceAdapter
    implements ListVariantsPort, LoadVariantPort, LoadSoftDeletedVariantPort, LoadAllVariantsPort,
    LoadVariantsForProductPort, LoadAllSoftDeletedVariantsPort, CreateVariantPort,
    CreateAllVariantsPort, UpdateVariantPort, UpdateAllVariantsPort,
    UpdateAllVariantsProductNameForProductPort, DeleteVariantPort, DeleteVariantsForProductPort {
  private static final Collector<Variant, ?, Map<VariantId, Variant>> COLLECTOR = Collectors
      .toUnmodifiableMap(Variant::getId, Function.identity(), (existing, replacement) -> existing);
  ...
  @Override
  public PageResponseDto<Variant> list(final ListVariantsQuery query) {
    final var pageable = PageRequests.toPageable(query.pageRequest(), VariantDocument.Fields.id);

    final Page<VariantDocument> page;
    if (query.targets().isEmpty()) {
      page = this.repository.findAllByDeletionTimeIsNull(pageable);
    } else {
      final var rawTargets = query.targets().stream().map(VariantTarget::value).toList();
      page = this.repository.findAllByTargetsInAndDeletionTimeIsNull(rawTargets, pageable);
    }

    final var variants = page.getContent().stream().map(this.mapper::toDomain).toList();

    return new PageResponseDto<>(variants, page.getNumber(), page.getSize(),
        page.getTotalElements());
  }

  @Override
  public Optional<Variant> loadById(final VariantId id) {
    final var jpaId = id.value();
    return this.repository.findByIdAndDeletionTimeIsNull(jpaId).map(this.mapper::toDomain);
  }

  @Override
  public Optional<Variant> loadSoftDeletedById(final VariantId id) {
    final var jpaId = id.value();
    return this.repository.findByIdAndDeletionTimeIsNotNull(jpaId).map(this.mapper::toDomain);
  }

  @Override
  public Map<VariantId, Variant> loadAllByIds(final Set<VariantId> ids) {
    ...
    return this.repository.findAllByIdInAndDeletionTimeIsNull(jpaIds).stream()
        .map(this.mapper::toDomain).collect(VariantPersistenceAdapter.COLLECTOR);
  }

  @Override
  public List<Variant> loadAllByProductId(final VariantProductId productId) {
    final var jpaProductId = productId.value();
    return this.repository.findAllByProductId(jpaProductId).stream().map(this.mapper::toDomain)
        .toList();
  }

  @Override
  public List<Variant> loadAllSoftDeletedByIds(final Collection<VariantId> ids) {
    final var jpaIds = ids.stream().map(VariantId::value).toList();
  ...
  @Override
  public Variant create(final NewVariant newVariant) {
  ...
  @Override
  public List<Variant> createAll(final Collection<NewVariant> newVariants) {
  ...
  @Override
  public List<Variant> updateAll(final Collection<Variant> variants) {
    final var toSave = variants.stream().map(this.mapper::toPersistence).toList();
    final var saved = this.repository.saveAll(toSave);
    return saved.stream().map(this.mapper::toDomain).toList();
  }

  @Override
  public void updateProductNameByProductId(final VariantProductId productId,
      final VariantProductName productName) {
    final var query =
        Query.query(Criteria.where(VariantDocument.Fields.productId).is(productId.value()));
  ...
  @Override
  public void deleteById(final VariantId id) {
