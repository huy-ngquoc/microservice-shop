...
public class UpdateVariantInfoService implements UpdateVariantInfoUseCase {
  ...
  public VariantView updateInfo(final UpdateVariantInfoCommand command) {
    ...
    final var soldCount = this.loadSoldCountPort.loadByIdOrZero(id, variant.getProductId());
    final var stockCount = this.loadStockCountPort.loadByIdOrZero(id, variant.getProductId());
    ...
    if ((priceSet == null) && (traitsSet == null) && (targetsSet == null)) {
    ...
    final var next =
        UpdateVariantInfoService.applyChanges(variant, priceSet, traitsSet, targetsSet);
    if (next == null) {
      return this.mapper.toView(variant, soldCount, stockCount);
    }
    ...
    return this.mapper.toView(saved, soldCount, stockCount);
  }

  private static @Nullable Variant applyChanges(final Variant current,
    ...
    return new Variant(current.getId(), current.getProductId(), current.getProductName(), newPrice,
        newTraits, newTargets, current.getImageKey(), current.getVersion(),
