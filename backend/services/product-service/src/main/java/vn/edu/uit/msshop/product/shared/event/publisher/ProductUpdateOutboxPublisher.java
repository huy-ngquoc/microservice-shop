...
public class ProductUpdateOutboxPublisher {
  ...
  public void publishPendingEvents() {
    List<ProductUpdateDocument> pendingEvents =
        productUpdateRepo.findTop50ByEventStatusOrderByCreatedAtAsc("PENDING");

    for (ProductUpdateDocument event : pendingEvents) {
      try {
        ProductUpdate productUpdate =
            new ProductUpdate(event.getEventId(), event.getProductId(), event.getName());
        kafkaTemplate.send(PUBLISH_TOPIC, productUpdate).whenComplete((result, ex) -> {
          if (ex == null) {
  ...
  private void updateStatus(ProductUpdateDocument event, String status, @Nullable String error) {
  ...
  private void handleFailure(ProductUpdateDocument event, @Nullable String error) {
