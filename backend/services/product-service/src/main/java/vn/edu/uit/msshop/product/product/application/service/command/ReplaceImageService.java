...
@Slf4j
public class ReplaceImageService implements ReplaceProductImageUseCase {
    ...
    public void replaceImage(ReplaceImageCommand command) {
        Product product = loadPort.loadById(command.productId())
                .orElseThrow(() -> new ProductNotFoundException(command.productId()));
        final Product toSaveProduct = product.replaceKey(command.oldKey(), command.newKey());
