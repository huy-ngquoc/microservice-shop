...
public class VariantKafkaProducerListener
  ...
  @Override
  public void onSuccess(ProducerRecord<String, VariantIntegrationEvent> producerRecord,
      RecordMetadata recordMetadata) {
    log.debug("Sent event successfully: {} | Offset: {}",
        producerRecord.value().getClass().getSimpleName(), recordMetadata.offset());
  }

  @Override
  public void onError(ProducerRecord<String, VariantIntegrationEvent> producerRecord,
    ...
    if (recordMetadata != null) {
      log.error("Failed to send event: {} | Topic: {} | Partition: {}", eventName,
          recordMetadata.topic(), recordMetadata.partition(), exception);

      return;
    }

    log.error("Failed to send event: {} | Target Topic: {} | Key: {}", eventName,
