...
public class Payment {
    ...
    public static Payment create(
        ...
        return Payment.builder().paymentId(d.paymentId()).createAt(d.createAt()).currency(d.currency())
                .orderId(d.orderId()).paymentMethod(d.paymentMethod())
                .paymentStatus(d.paymentStatus()).paymentValue(d.paymentValue()).updateAt(d.updateAt())
                .userId(d.userId()).userEmail(d.userEmail).build();
    }
    ...
    public static Payment reconstitue(
        ...
        return Payment.builder().paymentId(s.paymentId()).createAt(s.createAt()).currency(s.currency())
                .orderId(s.orderId()).paymentMethod(s.paymentMethod())
                .paymentStatus(s.paymentStatus()).paymentValue(s.paymentValue()).updateAt(s.updateAt())
                .userId(s.userId()).userEmail(s.userEmail).build();
    }

    @NullMarked
    public Snapshot snapshot() {
        return Snapshot.builder().paymentId(this.paymentId).createAt(this.createAt).currency(this.currency)
                .orderId(this.orderId).paymentMethod(this.paymentMethod)
                .paymentStatus(this.paymentStatus).paymentValue(this.paymentValue).updateAt(this.updateAt)
                .userId(this.userId).userEmail(this.userEmail).build();

    }

    @NullMarked
    public Payment applyUpdateInfo(
            final UpdateInfo u) {
        if (u == null) {
        ...
        return Payment.builder().paymentId(paymentId).createAt(createAt).currency(u.currency()).orderId(orderId)
                .paymentMethod(u.paymentMethod())
                .paymentStatus(u.paymentStatus()).paymentValue(paymentValue).updateAt(new UpdateAt(Instant.now()))
                .userId(this.userId).userEmail(this.userEmail).build();
