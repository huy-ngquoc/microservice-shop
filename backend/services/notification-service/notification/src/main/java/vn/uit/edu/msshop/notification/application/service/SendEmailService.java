...
public class SendEmailService implements SendMailUseCase {
    ...
    public Email createEmail(
        ...
        Email email = new Email(new EmailId(UUIDs.newId()), command.getEmailContent(), new EmailStatus("UNSENT"),
                command.getEmailTitle(), command.getEmailType(), new SendTime(Instant.now()), new UpdateTime(null),
                command.getOrderId(), command.getUserEmail(), new CreationTime(Instant.now()), command.getUserId());
