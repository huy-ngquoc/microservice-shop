...
public class UpdateCartInfoService implements UpdateCartInfoUseCase {
    ...
    public void updateMany(
        ...
        for (UpdateCartInfoCommand command : commands) {
            UserId userId = command.userId();
            System.out.println("Processing User id: " + userId.value());

            Cart cart = loadCartPort.loadByUserId(userId);
            if (cart == null)
                continue;

            CartDetail.UpdateInfo u = getUpdateInfo(command, cart);
            if (u == null)
                continue;

            // Chuyển item đơn lẻ thành List và gọi hàm applyUpdateInfo gốc của Cart
            final var updateInfo = Cart.UpdateInfo.builder()
                    .userId(userId)
                    .detailUpdateInfos(List.of(u)) // Bọc vào List tại đây
                    .build();

            final var saved = cart.applyUpdateInfo(updateInfo);
            saveCartPort.save(saved);
        }
    }

    private CartDetail.UpdateInfo getUpdateInfo(
            UpdateCartInfoCommand command,
            Cart cart) {
        CartDetail detail = cart.findByVariantId(command.variantId());
        if (detail == null)
            return null;
        System.out.println("Price " + command.unitPrice().apply(detail.getPrice()).value());
        return CartDetail.UpdateInfo.builder().variantId(command.variantId())
                .imageKey(command.imageKey().apply(detail.getImageKey()))
                .name(command.name().apply(detail.getName()))
                .price(command.unitPrice().apply(detail.getPrice()))
                .traits(command.traits().apply(detail.getTraits()))
                .build();
    }

    private List<CartDetail.UpdateInfo> getUpdateDetailInfos(
            List<UpdateCartInfoCommand> commands,
            Cart cart) {
