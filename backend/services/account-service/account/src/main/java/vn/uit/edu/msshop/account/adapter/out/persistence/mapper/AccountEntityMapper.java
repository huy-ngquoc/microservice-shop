...
public class AccountEntityMapper {
    ...
    public AccountJpaEntity toEntity(
            Account a) {
        ...
        return AccountJpaEntity.of(snapshot.id().value(), snapshot.name().value(), snapshot.password().value(),
                snapshot.email().value(), snapshot.role().value(), snapshot.status().value(), url, publicId, width,
                height, snapshot.shippingAdress().value(), snapshot.phoneNumber().value(), keyCloakId);
    }
    ...
    public Avatar toAvatar(
            ...
            int width,
            int height) {
        return new Avatar(new AvatarPublicId(publicId == null ? "" : publicId), new AvatarUrl(url == null ? "" : url),
