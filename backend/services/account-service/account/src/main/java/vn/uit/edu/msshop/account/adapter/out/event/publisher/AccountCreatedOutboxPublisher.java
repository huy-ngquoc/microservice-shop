...
import vn.uit.edu.msshop.account.adapter.out.event.repositories.AccountCreatedDocumentRepository;
...
public class AccountCreatedOutboxPublisher {
    ...
    public void publishPendingEvents() {
        List<AccountCreatedDocument> pendingEvents = accountCreatedDocumentRepo
        ...
        for (AccountCreatedDocument event : pendingEvents) {
            try {
                AccountCreated accountCreated = new AccountCreated(event.getId(), event.getName(), event.getEmail(),
                        event.getPassword(), event.getRole(), event.getStatus(), event.getShippingAddress(),
                        event.getPhoneNumber(), event.getEventId());
