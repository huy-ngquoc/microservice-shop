...
public class AccountEventPublisherAdapter implements PublishAccountEventPort {
    ...
    private final KafkaTemplate<
            String,
            vn.uit.edu.msshop.account.domain.event.kafka.AccountCreated> accountCreatedTemplate;
    ...
    @Override
    public void sendAccountCreationFailEvent(
            AccountIdDocument outboxEvent) {
        AccountId accountId = new AccountId(outboxEvent.getAccontId(), outboxEvent.getEventId());
        Message<AccountId> message = MessageBuilder.withPayload(accountId).setHeader(KafkaHeaders.TOPIC, PUBLISH_TOPIC)
                .build();
        try {
    ...
    @Override
    public void sendDeleteOldImageEvent(
            DeleteOldImageEventDocument outboxEvent) {
        DeleteOldImageEvent event = new DeleteOldImageEvent(outboxEvent.getOldImagePublicId(),
                outboxEvent.getEventId());
        Message<DeleteOldImageEvent> message = MessageBuilder.withPayload(event)
                .setHeader(KafkaHeaders.TOPIC, PUBLISH_AVATAR_EVENT_TOPIC).build();
        try {
            kafkaDeleteOldAvatarTemplate.send(message).whenComplete((
                    result,
                    ex) -> {
                if (ex == null) {
    ...
    @Override
    public void sendRollbackImageEvent(
            RollbackImageEventDocument outboxEvent) {
        final var event = new RollbackImageEvent(outboxEvent.getImagePublicId(), outboxEvent.getEventId());
        Message<RollbackImageEvent> message = MessageBuilder.withPayload(event)
                .setHeader(KafkaHeaders.TOPIC, PUBLISH_AVATAR_EVENT_TOPIC).build();
        try {
            kafkaRollbackAvatarTemplate.send(message).whenComplete((
                    result,
                    ex) -> {
                if (ex == null) {
    ...
    public void sendAccountCreated(
            AccountCreatedDocument outboxEvent) {
        final var event = new vn.uit.edu.msshop.account.domain.event.kafka.AccountCreated(outboxEvent.getId(),
                outboxEvent.getName(), outboxEvent.getEmail(),
                outboxEvent.getPassword(), outboxEvent.getRole(), outboxEvent.getStatus(),
                outboxEvent.getShippingAddress(), outboxEvent.getPhoneNumber(), outboxEvent.getEventId());
        Message<vn.uit.edu.msshop.account.domain.event.kafka.AccountCreated> message = MessageBuilder.withPayload(event)
                .setHeader(KafkaHeaders.TOPIC, PUBLISH_ACCOUNT_CREATED_TOPIC).build();
        try {
            accountCreatedTemplate.send(message).whenComplete((
                    result,
                    ex) -> {
                if (ex == null) {
