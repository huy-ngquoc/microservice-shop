...
public class AccountViewMapper {
    public AccountView toView(
        ...
        return new AccountView(account.getId().toString(), account.getName().value(),
                account.getEmail().value(), account.getPassword().value(), account.getRole().value(),
                account.getStatus().value(), account.getShippingAddress().value(), account.getAvatar().url().value(),
                account.getPhoneNumber().value(),
                account.getKeyCloakId().value() != null ? account.getKeyCloakId().value().toString() : "");
    }
