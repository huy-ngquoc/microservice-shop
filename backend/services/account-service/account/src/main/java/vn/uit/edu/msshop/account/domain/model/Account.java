...
public class Account {
    ...
    public static Account create(
        ...
        return Account.builder().id(d.id()).name(d.name()).email(d.email()).password(d.password()).role(d.role())
                .status(d.status()).avatar(null).shippingAddress(d.shippingAddress()).phoneNumber(d.phoneNumber())
                .keyCloakId(new KeyCloakId(null))
                .build();
    }

    @NullMarked
    public static Account reconstitue(
            SnapShot s) {
        if (s == null) {
            throw new IllegalArgumentException("Draft must not be null");
        }
        ...
        return Account.builder().id(s.id()).name(s.name()).email(s.email()).password(s.password()).role(s.role())
                .status(s.status()).avatar(s.avatar()).shippingAddress(s.shippingAdress()).phoneNumber(s.phoneNumber())
                .keyCloakId(s.keyCloakId())
                .build();
    }

    @NullMarked
    public Account applyUpdateInfo(
            UpdateInfo u) {
        if (u == null) {
        ...
        return Account.builder().id(this.id).name(u.name()).email(u.email()).password(u.password()).role(u.role())
                .status(u.status()).avatar(this.avatar).shippingAddress(u.shippingAddress())
    ...
    public SnapShot snapShot() {
        return SnapShot.builder().id(this.id).name(this.name).email(this.email).password(this.password).role(this.role)
                .status(this.status).avatar(this.avatar).shippingAdress(this.shippingAddress)
    ...
    private boolean isSameInfoWithUpdateInfo(
            UpdateInfo u) {
        return Objects.equals(u.name(), this.name) && Objects.equals(u.email(), this.email)
                && Objects.equals(u.password(), this.password) && Objects.equals(u.role(), this.role)
                && Objects.equals(u.status(), this.status) && Objects.equals(u.shippingAddress(), this.shippingAddress)
