...
public class RatingInfo {
    ...
    @Builder
    public static record Draft(ProductId productId, RatingCount ratingCount, RatingPoint totalPoint) {

    }
    @Builder
    public static record Snapshot(ProductId productId, RatingCount ratingCount, RatingPoint totalPoint) {
    ...
    public static RatingInfo create(Draft d) {
        ...
        return RatingInfo.builder().productId(d.productId()).ratingCount(d.ratingCount()).totalPoint(d.totalPoint()).build();
    }
    public static RatingInfo reconstitue(Snapshot s) {
        if(s.productId==null)  throw new IllegalArgumentException("Invalid id");
        return RatingInfo.builder().productId(s.productId()).ratingCount(s.ratingCount()).totalPoint(s.totalPoint()).build();
    }
    public RatingInfo applyUpdateInfo(UpdateInfo updateInfo) {
        ...
        return RatingInfo.builder().productId(this.productId).ratingCount(updateInfo.ratingCount).totalPoint(updateInfo.totalPoint()).build();
    }
    ...
    public RatingInfo increaseRatingPoint(RatingPoint ratingPoint) {
        ...
        return RatingInfo.builder().productId(this.productId).ratingCount(new RatingCount(this.ratingCount.value()+1)).totalPoint(new RatingPoint(nextPoint)).build();
    }
    public RatingInfo decreaseRatingPoint(RatingPoint ratingPoint) {
        ...
        return RatingInfo.builder().productId(this.productId).ratingCount(new RatingCount(this.ratingCount.value()+1)).totalPoint(new RatingPoint(prevPoint)).build();
    }
 }
