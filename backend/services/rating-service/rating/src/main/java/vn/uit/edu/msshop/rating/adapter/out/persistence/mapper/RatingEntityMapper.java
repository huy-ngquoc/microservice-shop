...
public class RatingEntityMapper {
    public Rating toDomain(
        ...
        final var snapshot = Rating.Snapshot.builder().id(new RatingId(entity.getId()))
    ...
    public RatingMongoDBEntity toEntity(
        ...
        if (rating.getMedia() == null) {
            ...
            media = new MediaDocument(rating.getMedia().type(), rating.getMedia().url(), rating.getMedia().size(),
                    rating.getMedia().publicId());
        }
        return new RatingMongoDBEntity(rating.getId().value(), rating.getProductId().value(),
                rating.getUserId().value(), rating.getContent().value(), media, rating.getUsername().value(),
                rating.getRatingPoint().value(), rating.getUserAvatar().value());
    }
}
