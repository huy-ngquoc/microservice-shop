...
public class RatingInfoViewMapper {
    public RatingInfoView toView(
            RatingInfo domain) {
        return new RatingInfoView(
                domain.getProductId().value(),
                domain.getRatingCount().value(),
                domain.getRatingTotal().value(),
                domain.getCreateAt().value(),
                domain.getUpdateAt().value());
    }
}
