...
public class InventoryJpaMapper {
    public InventoryJpaEntity toEntity(
            Inventory domain) {
        return new InventoryJpaEntity(domain.getId().value(), domain.getVariantId().value(),
                domain.getQuantity().value(), domain.getReservedQuantity().value(), domain.getLastUpdate().value(),
                domain.getStatus().value(), domain.getCreateAt().value(), domain.getVersion().value());
    }
    ...
    public InventoryJpaEntity toNew(
            VariantId variantId) {
        return InventoryJpaEntity.of(variantId.value(), 0, 0, null, "ENABLE");
    }

    public InventoryJpaEntity toNewFromCommand(
            VariantId variantId,
            Quantity quantity) {
