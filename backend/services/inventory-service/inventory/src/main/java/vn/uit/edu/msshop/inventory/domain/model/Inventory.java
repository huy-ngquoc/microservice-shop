...
public class Inventory {
    ...
    public static Inventory create(
        ...
        return Inventory.builder().id(draft.id()).variantId(draft.variantId()).quantity(draft.quantity())
                .reservedQuantity(draft.reservedQuantity()).lastUpdate(new LastUpdate(null))
                .status(new InventoryStatus("ENABLE")).version(new Version(0)).createAt(new CreateAt(Instant.now()))
                .build();
    }

    public static Inventory reconstitue(
            Snapshot s) {
        if (s == null)
            throw new IllegalArgumentException("Invalid snapshot");
        return Inventory.builder().id(s.id()).variantId(s.variantId()).quantity(s.quantity())
                .reservedQuantity(s.reservedQuantity()).lastUpdate(s.lastUpdate()).version(s.version())
                .status(s.status()).createAt(s.createAt).build();
    }

    public Snapshot snapshot() {
        return Snapshot.builder().id(this.id).variantId(this.variantId).quantity(this.quantity)
                .reservedQuantity(this.reservedQuantity).lastUpdate(this.lastUpdate).status(this.status)
                .createAt(this.createAt).build();
    }

    public Inventory applyUpdateInfo(
            UpdateInfo u) {
        if (u == null)
            throw new IllegalArgumentException("Update info must not be null");
        if (isTheSameInfoWithUpdate(u))
            return this;
        return Inventory.builder().id(this.id).variantId(this.variantId).quantity(u.quantity())
                .reservedQuantity(u.reservedQuantity()).lastUpdate(new LastUpdate(Instant.now())).version(this.version)
                .status(u.status).createAt(this.createAt).build();
    }

    private boolean isTheSameInfoWithUpdate(
            UpdateInfo u) {
        return u.quantity().value() == this.quantity.value()
                && u.reservedQuantity().value() == this.reservedQuantity.value();
    }
    ...
    public Inventory disable() {
        return Inventory.builder().id(this.id).variantId(this.variantId).quantity(this.quantity)
                .reservedQuantity(this.reservedQuantity).lastUpdate(new LastUpdate(Instant.now())).version(this.version)
                .status(new InventoryStatus("DISABLE")).build();
    }

    public Inventory restore() {
        return Inventory.builder().id(this.id).variantId(this.variantId).quantity(this.quantity)
                .reservedQuantity(this.reservedQuantity).lastUpdate(new LastUpdate(Instant.now())).version(this.version)
                .status(new InventoryStatus("ENABLE")).build();
