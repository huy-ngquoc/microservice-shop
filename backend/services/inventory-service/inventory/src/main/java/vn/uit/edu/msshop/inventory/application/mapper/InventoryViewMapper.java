...
public class InventoryViewMapper {
    public InventoryView toView(Inventory inventory) {
        return new InventoryView(inventory.getId().value(), inventory.getVariantId().value(), inventory.getQuantity().value(), inventory.getReservedQuantity().value(), inventory.getLastUpdate().value(), inventory.getStatus().value());
    }
}
